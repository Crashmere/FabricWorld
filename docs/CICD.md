...
Playwright 的 chromium 项目运行全部流程；webkit-purchase 和 webkit-materials 项目使用 iPhone 视口分别运行 purchase.spec.ts 与 materials.spec.ts。WebKit 自动化仍是桌面构建，不等同真实 iOS。日期回归额外模拟 padding 被加到百分比宽度的盒模型，以覆盖 WebKit 301648 对应的布局风险。
