...
| 材质 | 常见纤维多选并允许自定义，如棉、麻、羊毛、涤纶；单选默认为 100%，多选在各项前输入百分比；成分说明保留独立补充文本 |
...
| 材质管理 | 可搜索材质列表、布料数量和批量移除确认 | 居中列表，提供相同操作 |
...
材质管理目录直接从全部当前记录（含回收站）生成，无独立材质表。批量移除按精确名称匹配，以相关记录的 ID/revision 集合摘要校验版本；在一个事务更新全部记录、修改历史和幂等结果。目录过期返回 409 并重新确认，未知结果保存原请求键供查询/重试及刷新恢复。内置但未使用的建议不列入目录，已保存自定义材质可在编辑器复用。
