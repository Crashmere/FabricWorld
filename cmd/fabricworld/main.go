...
import (
	...
	"flag"
	"fmt"
	"github.com/Crashmere/FabricWorld/internal/app"
	"github.com/Crashmere/FabricWorld/web"
	"io/fs"
	"log"
	"net/http"
	"os"
...
func run() error {
	...
	listen := flags.String("listen", "127.0.0.1:18082", "HTTP address")
	out := flags.String("out", "", "backup directory or restore destination")
	source := flags.String("source", "", "backup to restore")
	prefix := flags.Bool("with-prefix", false, "serve /fabricworld/ directly (local testing)")
	...
	if *prefix {
		...
		mux.Handle("/fabricworld/", http.StripPrefix("/fabricworld", handler))
		handler = mux
	}
	...
	log.Printf("FabricWorld listening on %s", *listen)
