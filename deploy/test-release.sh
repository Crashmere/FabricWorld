...
release_test=$(mktemp -d /tmp/fabricworld-release-test.XXXXXX)
...
sed -e 's|^export PATH=.*|export PATH="$TEST_COMMANDS:/usr/bin:/bin"|' \
  ...
  -e 's|^app=/opt/fabricworld$|app=$TEST_APP|' \
  -e 's|^exec 9>/run/lock/fabricworld-deploy.lock$|exec 9>"$TEST_APP/deploy.lock"|' \
...
#!/usr/bin/env bash
set -euo pipefail
if grep -q '^# unhealthy$' "$TEST_APP/bin/fabricworld"; then exit 22; fi
printf '%s\n' '{"status":"ok"}' '/fabricworld/assets/test.js'
SH
cat > "$TEST_COMMANDS/runuser" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
[[ $1 == -u && $2 == fabricworld && $3 == -- ]] || exit 92
shift 3
exec "$@"
SH
...
for scenario in checksum bad-check unhealthy good; do
  ...
  chmod 0755 "$TEST_APP/bin/fabricworld"
  printf 'synthetic database\n' > "$TEST_APP/data/fabricworld.db"
  cp "$TEST_APP/data/fabricworld.db" "$TEST_APP/db-before"
  ...
  if [[ $scenario == good ]]; then
    ...
    cmp "$candidate" "$TEST_APP/bin/fabricworld"
    [[ $(cat "$TEST_APP/current-commit") == "$commit" ]]
    grep -qx success "$TEST_APP/releases/"*/result
  else
    [[ $result != 0 ]]
    cmp "$release_test/previous" "$TEST_APP/bin/fabricworld"
  ...
  cmp "$TEST_APP/db-before" "$TEST_APP/data/fabricworld.db"
